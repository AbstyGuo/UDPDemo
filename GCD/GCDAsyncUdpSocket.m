...
- (BOOL)createSocket4:(BOOL)useIPv4 socket6:(BOOL)useIPv6 error:(NSError **)errPtr
{
	...
	int(^createSocket)(int) = ^int (int domain) {
        ...
        status = setsockopt(socketFD, SOL_SOCKET,SO_SNDBUF,(char*)&optVal,optLen );
        
         optVal = 1024 * 1024;
         optLen = sizeof(int);

        status = setsockopt(socketFD, SOL_SOCKET,SO_RCVBUF,(char*)&optVal,optLen );
